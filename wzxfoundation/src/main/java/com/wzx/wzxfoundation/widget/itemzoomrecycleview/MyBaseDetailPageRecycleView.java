package com.wzx.wzxfoundation.widget.itemzoomrecycleview;
...
import com.wzx.wzxfoundation.base.BaseActivity;
...
public class MyBaseDetailPageRecycleView extends RecyclerView {
    ...
    public MyBaseDetailPageRecycleView(@NonNull Context context) {
        super(context);
    }

    public MyBaseDetailPageRecycleView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public MyBaseDetailPageRecycleView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyle) {
    ...
    public boolean dispatchTouchEvent(MotionEvent event) {
        if (mActivity != null) {
            ...
            if (mState == 1) {//准备
                LogHelper.d("wangzixu", "detailpage onTouch state1 action = " + action + ", event.getPointerCount() = " + event.getPointerCount());
                ...
                if (action == MotionEvent.ACTION_DOWN) {
                    mDonwX = event.getX();
                    mDownY = event.getY();
                    LogHelper.d("wangzixu", "detailpage onTouch dispatchTouchEvent mDonwX  = " + mDonwX + ", mDownY = " + mDownY);
    ...
    private ImageView findUnderImageView(ViewGroup parent, int x1, int y1, int x2, int y2) {
        for (int i = parent.getChildCount(); i >= 0; i--) {
            ...
            if (child != null && child.getId() == mOriId && child instanceof ImageView) {
                ...
                //LogHelper.d("wangzixu", "detailpage onTouch view1.findViewById mOriLocation  xy =  " + x + "," + y);
